...
interface InvestmentCashFlowProps {
  data: Array<{ category: string; amount: number; type: string }>;
  loading?: boolean;
}

export function InvestmentCashFlow({ data, loading }: InvestmentCashFlowProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <Card className="p-6">
        <EmptyState message="No investment data" />
      </Card>
    );
  }
