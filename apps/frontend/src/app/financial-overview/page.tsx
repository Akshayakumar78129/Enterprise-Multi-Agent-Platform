...
import { useFinancialoverviewContext } from './context';
import { useFinancialoverviewData } from './hooks/useFinancialoverviewData';

export default function FinancialoverviewPage() {
  const { filters } = useFinancialoverviewContext();
  const { loading, error, data, kpiMetrics, hasNoData } = useFinancialoverviewData(filters);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        <DashboardSection title="AI Assistant">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              AI Chatbot for financial overview insights will be integrated here
            </div>
          </div>
        </DashboardSection>
        ...
        <DashboardSection title="BI Panel">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              Business Intelligence panel for financial overview analysis will be integrated here
            </div>
          </div>
        </DashboardSection>
      </div>
