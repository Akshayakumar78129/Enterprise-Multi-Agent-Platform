...
import { useMarketinsightsContext } from './context';
import { useMarketinsightsData } from './hooks/useMarketinsightsData';

export default function MarketinsightsPage() {
  const { filters } = useMarketinsightsContext();
  const { loading, error, data, kpiMetrics, hasNoData } = useMarketinsightsData(filters);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        <DashboardSection title="AI Assistant">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              AI Chatbot for market insights insights will be integrated here
            </div>
          </div>
        </DashboardSection>
        ...
        <DashboardSection title="BI Panel">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              Business Intelligence panel for market insights analysis will be integrated here
            </div>
          </div>
        </DashboardSection>
      </div>
