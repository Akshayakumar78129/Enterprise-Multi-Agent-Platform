...
import { useSupplierperformanceContext } from './context';
import { useSupplierperformanceData } from './hooks/useSupplierperformanceData';

export default function SupplierperformancePage() {
  const { filters } = useSupplierperformanceContext();
  const { loading, error, data, kpiMetrics, hasNoData } = useSupplierperformanceData(filters);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        <DashboardSection title="Thought Catalyst">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              AI Chatbot for supplier performance insights will be integrated here
            </div>
          </div>
        </DashboardSection>
        ...
        <DashboardSection title="Decision Intelligence">
          <div className="h-[400px] border border-border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">
              Business Intelligence panel for supplier performance analysis will be integrated here
            </div>
          </div>
        </DashboardSection>
      </div>
