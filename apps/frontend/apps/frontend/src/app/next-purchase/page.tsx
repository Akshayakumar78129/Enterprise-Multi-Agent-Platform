// Next Purchase Predictor Dashboard Page - Following churn pattern
"use client";
...
export default function NextPurchasePage() {
  const { filters, setFilters, timeRange, setTimeRange, selectedPoints, selectionManager, customers, setCustomers } = useNextPurchaseContext();
  ...
  const handleDataPointClick = (dataPoint: any, event: React.MouseEvent) => {
    if (event?.ctrlKey || event?.metaKey) {
      ...
      showInsight({
        ...
        source: "Next Purchase Predictor"
      }, event);
    } else {
    ...
    <>
      ...
      <DashboardSection title="Key Metrics">
        {loading ? (
          ...
          <NextPurchaseKPIs data={data} loading={loading} />
        )}
      </DashboardSection>

      {/* Main Analysis Section */}
      <div id="main-analysis" />
      <DashboardSection title="Next Purchase Predictor Analysis">
        <NextPurchaseCharts
          data={chartData}
          loading={loading}
