"""
Transaction Patterns Processing Service
...
class TransactionPatternsService:
    """Processing service for Transaction pattern analysis using anomaly detection"""
    ...
    async def get_dashboard_summary(self, filters: Dict = {}) -> Dict:
        ...
        try:
            ...
            # Convert to DataFrames - Database returns 'rows' key, not 'data'
    ...
    def _perform_ml_analysis(self, df: pd.DataFrame, transaction_df: pd.DataFrame = None) -> Dict:
        ...
        if 'transaction_patterns' == 'customer_segmentation':
            return self.ml_predictor.perform_segmentation(df)
        elif 'transaction_patterns' == 'customer_ltv':
            return self.ml_predictor.predict_ltv(df)
        elif 'transaction_patterns' == 'engagement_classifier':
            return self.ml_predictor.classify_engagement(df)
        elif 'transaction_patterns' == 'next_purchase' and transaction_df is not None:
    ...
    def _calculate_kpis(self, customers_df: pd.DataFrame, transactions_df: pd.DataFrame,
        ...
        elif 'transaction_patterns' == 'purchase_frequency':
            kpis = {
    ...
    def _generate_visualizations(self, customers_df: pd.DataFrame, transactions_df: pd.DataFrame,
        ...
        # For transaction patterns dashboard, always use default visualizations
        visualizations = {
    ...
    def _create_product_metrics(self, transactions_df: pd.DataFrame) -> List[Dict]:
        """Create product-specific metrics for ProductMatrix component"""
        ...
        # Check if we have product category column
        if 'item_number' in transactions_df.columns or 'product_category' in transactions_df.columns:
            # Group by product category or item
            group_col = 'product_category' if 'product_category' in transactions_df.columns else 'item_number'

            product_groups = transactions_df.groupby(group_col).agg({
                'net_sales_amount': ['sum', 'mean'],
                'customer_id': 'count'
            }).reset_index()

            # Flatten column names
            product_groups.columns = [group_col, 'totalRevenue', 'avgPrice', 'transactionCount']

            # Sort by revenue and get top 20
            product_groups = product_groups.nlargest(20, 'totalRevenue')

            for _, row in product_groups.iterrows():
                product_metrics.append({
                    'productCategory': str(row[group_col]) if row[group_col] else 'Unknown',
                    'transactionCount': int(row['transactionCount']),
                    'totalRevenue': float(row['totalRevenue']),
                    'avgPrice': float(row['avgPrice']),
                    'growthRate': np.random.uniform(-5, 15)  # Mock growth rate for now
                })
