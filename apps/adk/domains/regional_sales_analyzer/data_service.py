...
class RegionalSalesAnalyzerDataService:
    ...
    async def get_regional_sales_data(self, filters: Dict[str, Any] = {}) -> List[Dict]:
        ...
        SELECT
            ...
            ROUND(SUM({self.schema.TRANSACTION.refs['sales_amount']}) * 1.0, 2) as totalSales,
            ROUND(SUM({self.schema.TRANSACTION.refs['net_sales_amount']}) * 1.0, 2) as netSales,
            ROUND(SUM({self.schema.TRANSACTION.refs['sales_quantity']}) * 1.0, 2) as totalQuantity,
            ROUND(SUM({self.schema.TRANSACTION.refs['gross_profit']}) * 1.0, 2) as grossProfit,
            COUNT(DISTINCT {self.schema.TRANSACTION.refs['customer_key']}) as customerCount,
            COUNT(*) as transactionCount,
            ROUND(AVG({self.schema.TRANSACTION.refs['sales_amount']}) * 1.0, 2) as avgTransactionValue,
    ...
    async def get_country_level_data(self, filters: Dict[str, Any] = {}) -> List[Dict]:
        ...
        SELECT
            ...
            ROUND(SUM({self.schema.TRANSACTION.refs['sales_quantity']}) * 1.0, 2) as totalQuantity,
            ROUND(SUM({self.schema.TRANSACTION.refs['gross_profit']}) * 1.0, 2) as grossProfit,
            ROUND(SUM({self.schema.TRANSACTION.refs['gross_profit']}) * 1.0 / NULLIF(SUM({self.schema.TRANSACTION.refs['sales_amount']}), 0) * 100, 2) as profitMargin,
            COUNT(DISTINCT {self.schema.TRANSACTION.refs['customer_key']}) as customerCount,
            COUNT(*) as transactionCount,
