...
from domains.customer_behavior.data_service import CustomerBehaviorDataService
from domains.customer_behavior.ml_predictor import CustomerBehaviorMLPredictor
from utils.decorators import cache_dashboard_endpoint
...
class CustomerBehaviorProcessingService:
    ...
    async def get_behavior_data(self, filters: Dict) -> pd.DataFrame:
        ...
        try:
            ...
            date_from, date_to, loyalty_filters = self._separate_filters(filters)
            ...
            if loyalty_filters.get('segment_id'):
                filter_clauses.append(f"customer_type_id = ${param_index}")
                params.append(loyalty_filters['segment_id'])
                param_index += 1

            if loyalty_filters.get('customer_ids'):
                placeholders = ','.join([f'${i}' for i in range(param_index, param_index + len(loyalty_filters['customer_ids']))])
                filter_clauses.append(f"customer_id IN ({placeholders})")
                params.extend(loyalty_filters['customer_ids'])
                param_index += len(loyalty_filters['customer_ids'])

            if loyalty_filters.get('loyalty_status'):
                placeholders = ','.join([f'${i}' for i in range(param_index, param_index + len(loyalty_filters['loyalty_status']))])
                filter_clauses.append(f"loyalty_status IN ({placeholders})")
                params.extend(loyalty_filters['loyalty_status'])
                param_index += len(loyalty_filters['loyalty_status'])
            ...
            result = await self.data_service.database.execute_query(query, params)
            return pd.DataFrame(result.get('rows', result.get('data', [])))
            ...
            if 'preferred_category' in behavior_df.columns:
                ...
                for cat, count in category_counts.items():
                    cat_data = {
                        ...
                        'avgSpend': np.random.uniform(100, 1000),
                        'sales': count * np.random.uniform(100, 1000)
                    }
