...
from .models import SegmentationFilters, SegmentationResponse
...
@router.post("/summary")
async def get_segmentation_summary(filters: SegmentationFilters = SegmentationFilters()) -> Dict:
    """
    ...
    """
    try:
        logger.info(f"Received segmentation request with filters: {filters.dict()}")

        # Convert filters to dict
        filter_dict = filters.dict(exclude_none=True)

        # Get segmentation summary
        result = await service.get_segmentation_summary(filter_dict)
