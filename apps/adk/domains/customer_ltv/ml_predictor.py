"""
ML predictor for Customer Lifetime Value
...
class CustomerLtvMLPredictor:
    """ML predictor for Customer lifetime value prediction using regression models"""

    def __init__(self):
        """Initialize the ML predictor"""
        self.model = None
        self.scaler = StandardScaler()
        self.feature_cols = ['total_purchases', 'avg_order_value', 'purchase_frequency', 'customer_age_days', 'retention_rate']
    ...
    def predict_ltv(self, df: pd.DataFrame) -> Dict:
        ...
        # Prepare features
        features_df = self._prepare_ltv_features(df)

        # Train model if not already trained
        if not self.is_trained:
            self._train_ltv_model(features_df, df['lifetime_value'])
        ...
        df['ltv_percentile'] = pd.qcut(predictions, q=4, labels=['Low', 'Medium', 'High', 'VIP'])

        return {
            'predictions': df[['customer_id', 'customer_name', 'predicted_ltv', 'ltv_percentile']].to_dict('records'),
            'feature_importance': self._get_feature_importance(),
            'ltv_distribution': self._get_ltv_distribution(df),
            'high_value_customers': self._get_high_value_customers(df)
        }

    def _prepare_ltv_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Prepare features for LTV prediction"""

        features_df = pd.DataFrame()

        # Transaction-based features
        features_df['total_purchases'] = df.get('transaction_count', 0)
        features_df['avg_order_value'] = df.get('avg_transaction_value', 0).fillna(0)
        features_df['purchase_frequency'] = features_df['total_purchases'] / df.get('customer_lifetime_days', 1).replace(0, 1)

        # Customer age
        features_df['customer_age_days'] = df.get('customer_lifetime_days', 0)

        # Retention rate (using loyalty data)
        features_df['retention_rate'] = (365 - df.get('days_since_last_activity', 365).clip(upper=365)) / 365
