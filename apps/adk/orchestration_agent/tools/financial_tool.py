...
def get_db_path():
    ...
    # Override with fixed path to ensure consistency
...
def cash_flow_analysis(
    ...
    try:
        ...
        # Save visualization
        reports_path = Path(os.path.join(project_root, "data", "reports"))
...
def analyze_ar_aging(
    ...
    try:
        ...
        # Query AR data - using square brackets for table name
        query = """
        ...
        if len(ar_data) == 0:
            return json.dumps({"error": "No accounts receivable data found"})
        ...
        customer_counts = ar_data.groupby('Customer ID').size().reset_index(name='count')
        customers_with_overdue = ar_data[ar_data['Days Overdue'] > 30]['Customer ID'].nunique()
        total_customers = ar_data['Customer ID'].nunique()
        ...
        high_value_accounts = high_value_overdue.groupby('Customer ID')['Balance Due Amount'].sum().reset_index()
        ...
        result.append("=" * 80)
        result.append(f"Report Date: {max_date.strftime('%Y-%m-%d')}")
        result.append("=" * 80)
        ...
        if len(high_value_accounts) > 0:
            ...
            for idx, row in high_value_accounts.head(5).iterrows():
                result.append(f"Customer ID {row['Customer ID']}: ${row['Balance Due Amount']:,.2f}")
