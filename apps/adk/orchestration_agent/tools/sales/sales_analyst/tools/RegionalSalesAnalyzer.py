...
DB_PATH =Path(__file__).parent.parent.parent.parent.joinpath('database', 'sales_agent.db')
# VISUALIZATION_PATH = r"C:\Code\PythonProject\MultiagentML\multiagent-googleADK\orchestration_agent\output\visualizations"
VISUALIZATION_PATH = Path(__file__).parent.parent.parent.parent.joinpath('output', 'visualizations')
# Import utility functions
from orchestration_agent.tools.sales.sales_analyst.utils.regional_data_utils import (
    prepare_regional_data, 
    get_regional_filters, 
    apply_regional_filters,
    create_geo_hierarchy
)
from orchestration_agent.tools.sales.sales_analyst.utils.regional_performance_utils import (
    analyze_regional_performance,
    analyze_region_time_series,
    compare_regions,
    identify_regional_opportunities
)
from orchestration_agent.tools.sales.sales_analyst.utils.regional_visualization_utils import (
...
def get_sales_data(time_period: str = None,
    ...
    try:
        ...
        try:
            ...
            df['margin_pct'] = (df['gross_profit_amount'] / df['net_sales_amount'].replace(0, 1)) * 100  # Replace 0 with 1 to avoid division by zero
            df['revenue_per_unit'] = df['net_sales_amount'] / df['net_sales_quantity'].replace(0, 1)  # Replace 0 with 1 to avoid division by zero
...
def test_regional_sales_analysis():
    ...
    try:
        ...
        from orchestration_agent.tools.sales.sales_analyst.utils.regional_data_utils import prepare_regional_data
        ...
        from orchestration_agent.tools.sales.sales_analyst.utils.regional_performance_utils import analyze_regional_performance
        result = analyze_regional_performance(
            prepared_data,
            metric="revenue",
            include_growth=True
