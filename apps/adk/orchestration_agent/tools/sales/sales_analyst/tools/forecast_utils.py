...
try:
    from database.connector import DatabaseConnector, get_db_connector
...
def create_forecast_visualization(
    ...
    """
    Prepares data for Nivo line chart (JS), instead of plotting.
    Returns a list of series dicts for Nivo, e.g.:
    [
      {"id": "Forecast", "data": [{"x": date, "y": value}, ...]},
      {"id": "Historical Data", "data": [{"x": date, "y": value}, ...]},
      ...
    ]
    """
    ...
    if original_series is not None and column in original_series.columns:
        hist = {
            ...
            "data": [
                ...
                for date, val in zip(original_series.index, original_series[column]) if int(val) > 0 and int(date) > 0
            ]
        }
        nivo_data.append(hist)
    ...
    forecast = {
        ...
        "data": [
            {"x": int(date), "y": int(val)}
            for date, val in zip(forecast_dates, point_forecast) if int(val) > 0 and int(date) > 0
        ]
    }
    nivo_data.append(forecast)

    # Prediction intervals (optional, as two extra lines)
    # if show_intervals and lower_bound and upper_bound:
    #     lower = {
    #         "id": "Lower Bound",
    #         "data": [
    #             {"x": int(date), "y": int(val)}
    #             for date, val in zip(forecast_dates, lower_bound) if int(val) > 0 and int(date) > 0
    #         ]
    #     }
    #     upper = {
    #         "id": "Upper Bound",
    #         "data": [
    #             {"x": int(date), "y": int(val)}
    #             for date, val in zip(forecast_dates, upper_bound) if int(val) > 0 and int(date) > 0
    #         ]
    #     }
    #     nivo_data.extend([lower, upper])
