...
def calculate_inventory_levels(data: pd.DataFrame, sales_data: pd.DataFrame, min_stock_threshold: float) -> pd.DataFrame:
    ...
    # Calculate days of supply (avoid division by zero)
    merged_data["Days of Supply"] = merged_data["Current_Stock"] / merged_data["Average Daily Sales"].replace(0, 1)
    
    # Calculate stock level as percentage of average daily sales (avoid division by zero)
    merged_data["Stock Level %"] = merged_data["Current_Stock"] / (merged_data["Average Daily Sales"].replace(0, 1) * 30)  # 30 days
