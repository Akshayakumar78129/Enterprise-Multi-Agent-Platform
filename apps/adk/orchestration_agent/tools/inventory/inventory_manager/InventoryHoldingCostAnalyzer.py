...
from orchestration_agent.utils.connectInventoryDB import connect_inventory_db
# Setup logger
...
def analyze_holding_costs(
    ...
    try:
        ...
        db_connector = connect_inventory_db()
        ...
        visualizations = holding_costs_bar_chart_data(analysis_data)
        
        # Generate cost-saving recommendations
        recommendations = generate_recommendations(analysis_data)
        
        # Format the result as a detailed text report
        result = analysis_result + format_result(analysis_data, recommendations)
        ...
        return f"An error occurred while analyzing inventory holding costs: {str(e)}"
...
def generate_sample_data() -> pd.DataFrame:
    ...
    for product in products:
        for warehouse in warehouses:
            ...
            average_stock = current_stock * np.random.uniform(0.8, 1.2)  # Random variation around current stock
...
def holding_costs_bar_chart_data(analysis_data: pd.DataFrame):
    """
    Returns grouped data for a scatter/line/bar chart by category, in the form:
    [
      {"id": <group>, "data": [{"x": <x>, "y": <y>}, ...]},
      ...
    ]
    Groups by Item_Category, x is Item_Name, y is Total Holding Cost.
    """
    grouped = analysis_data.groupby("Item_Category")
    result = []
    for group, df in grouped:
        data = []
        for _, row in df.iterrows():
            data.append({"x": row["Item_Name"], "y": row["Total Holding Cost"]})
        result.append({"id": group, "data": data})
    return result
