...
from database.query_templates import get_date_range, get_latest_date
...
class SalesPerformanceAnalyzer:
    ...
    def __init__(self, dimension: str, time_period: str, metric: str, 
        ...
        self.db_path = db_path or os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'database', 'sales_agent.db')
    ...
    def analyze_performance(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.metric == "revenue":
                ...
                result = amount_sum / quantity_sum.replace(0, 1)  # Replace 0 with 1 to avoid division by zero
            elif self.metric == "growth":
                # Calculate growth rate
                result = grouped["amount"].sum().pct_change().fillna(0) * 100  # Fill NaN with 0
            elif self.metric == "margin":
                # Calculate margin (assuming we have cost data)
                amount_sum = grouped["amount"].sum()
                cost = grouped["quantity"].sum() * 10
                result = ((amount_sum - cost) / amount_sum.replace(0, 1) * 100).fillna(0)  # Fill NaN with 0
            ...
            visualization = self._create_visualization(data, query_params=None)
    ...
    def _create_visualization(self, data: pd.DataFrame, query_params: Optional[Dict] = None):
        """
        Prepare data for Nivo charts (JS) for sales performance metrics.
        Returns a dict of series for Nivo, e.g.:
        {
          "daily_sales_trend": [{"x": date, "y": value}, ...],
          "sales_by_dimension": [{"x": dim, "y": value}, ...],
          "sales_distribution": [{"x": bin, "y": freq}, ...],
          "sales_vs_quantity": [{"x": quantity, "y": sales_amount}, ...]
        }
        """
        ...
        nivo_data = {}
        # Daily sales trend
        if 'Txn Date' in data.columns and 'Sales Amount' in data.columns:
            daily_sales = data.groupby('Txn Date')['Sales Amount'].sum()
            nivo_data['daily_sales_trend'] = [
                {"x": int(date), "y": int(val)} for date, val in daily_sales.items() if int(val) > 0 and int(date) > 0
            ]
        # Sales by dimension
        if hasattr(self, 'dimension') and self.dimension and self.dimension in data.columns:
            dimension_sales = data.groupby(self.dimension)['Sales Amount'].sum()
            nivo_data['sales_by_dimension'] = [
                {"x": int(dim), "y": int(val)} for dim, val in dimension_sales.items() if int(val) > 0 and int(dim) > 0
            ]
        # Sales distribution (histogram)
        if 'Sales Amount' in data.columns:
            counts, bins = np.histogram(data['Sales Amount'], bins=20)
            bin_centers = 0.5 * (bins[:-1] + bins[1:])
            nivo_data['sales_distribution'] = [
                {"x": int(x), "y": int(y)} for x, y in zip(bin_centers, counts) if int(y) > 0 and int(x) > 0
            ]
        # Sales vs Quantity (scatter)
        if 'Quantity' in data.columns and 'Sales Amount' in data.columns:
            nivo_data['sales_vs_quantity'] = [
                {"x": int(q), "y": int(s)} for q, s in zip(data['Quantity'], data['Sales Amount']) if int(s) > 0 and int(q) > 0
            ]
        return nivo_data
