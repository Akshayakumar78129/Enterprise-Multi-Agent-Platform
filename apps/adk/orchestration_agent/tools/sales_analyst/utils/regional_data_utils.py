    ...
    try:
        ...
        try:
            if 'margin_pct' not in data.columns:
                data['margin_pct'] = (data['margin'] / data['revenue'].replace(0, 1)) * 100  # Replace 0 with 1 to avoid division by zero
            if 'revenue_per_unit' not in data.columns:
                data['revenue_per_unit'] = data['revenue'] / data['units'].replace(0, 1)  # Replace 0 with 1 to avoid division by zero
        except Exception as e:
