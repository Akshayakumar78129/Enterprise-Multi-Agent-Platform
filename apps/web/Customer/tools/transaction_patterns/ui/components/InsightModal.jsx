          ...
          <Section title="AI insight">
            {aiAudit && (
              <div style={{ fontSize: 12, color: '#9fb3c8', marginBottom: 6 }}>
                Handled by: <span style={{ color:'#a5b4fc' }}>{aiAudit.agentAuthor || 'orchestrator'}</span>
                {aiAudit.mentionDetected ? ' • via mention' : ''}
                {' • '}<span title={aiAudit.traceId}>at {new Date(aiAudit.dispatchedAt).toLocaleTimeString()}</span>
              </div>
            )}
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 12, minHeight: 80, color: '#d1d5db', marginBottom: 8 }}>
              {aiLoading ? 'Generating...' : (aiText ? renderFormatted(aiText) : 'Use the prompt box below or click Explain to generate a plain-language summary with strategies and next steps.')}
            </div>
